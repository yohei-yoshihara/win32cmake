...
auto logger = spdlog::basic_logger_mt("basic_logger", "win32_cmake.log");


void test() {
  spdlog::set_level(spdlog::level::debug);
  logger->debug("spdlog debug");
  logger->info("spdlog info");
  logger->warn("spdlog warning");
...
bool InitializeFramework(HWND hwnd) {
  ...
  if (FAILED(hr))
  {
    logger->error("failed to create ribbon framework, hr = {}", hr);
    return false;
  }
  return true;
}
